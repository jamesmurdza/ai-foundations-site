...
import { requestLoginCode, verifyLoginCode } from "@portal/lib/actions/auth-code";
...
/**
 * The sign-in card. The portal's landing page (`/`) is the login experience, so
 * this is shared by `/` and the legacy `/login` route (which the auth actions
 * still redirect into). `home` controls where the brand/back link points.
 */
export function LoginView({ sp }: { sp: LoginSearchParams }) {
      ...
      <div className="card w-full max-w-[440px] !p-8 fade-up">
        ...
        {!onCodeStep ? (
          <>
            ...
            <p className="meta-light text-[13px] mt-6 text-center">
          ...
          <>
            ...
            <div className="flex items-center justify-between mt-4 text-[13px]">
              <Link href="/" className="link">← Use a different email</Link>
