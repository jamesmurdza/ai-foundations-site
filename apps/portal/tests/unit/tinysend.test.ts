...
vi.mock("@/lib/env", () => ({ env: { tinysendApiKey: "test-key" } }));

import { importSubscribers } from "@/lib/tinysend";
...
describe("importSubscribers without an API key", () => {
  it("returns tinysend_not_configured and makes no request", async () => {
    vi.resetModules();
    vi.doMock("@/lib/env", () => ({ env: { tinysendApiKey: "" } }));
    const fetchMock = vi.fn();
    vi.stubGlobal("fetch", fetchMock);
    const { importSubscribers: imp } = await import("@/lib/tinysend");
